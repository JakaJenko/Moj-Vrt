...
namespace Vrt
{
    ...
    /// </summary>
    public sealed partial class IzbiraVrta : Page
    {
        int pomaknjenx = 0;
        int pomaknjeny = 0;
        int velikostx = 0;
        ...
        private async void Page_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                using (var statement = connection.Prepare(@"SELECT ime FROM vrt;"))
                {
            ...
            vrtovi.ItemsSource = zadeve;
        }

        private void vrtovi_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            drugiGrid.Children.Clear();

            using (var connection = new SQLiteConnection("baza_vrt.sqlite"))
            {
                using (var statement = connection.Prepare(@"SELECT ime, visina, dolzina, razdelitevX, razdelitevY, lok_rastl FROM vrt WHERE ime='" + vrtovi.SelectedItem.ToString() + "';"))
                {
                    while (statement.Step() == SQLiteResult.ROW)
                    {
        ...
        private void button1_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ZnacilnostiVrta.izbranVrt = vrtovi.SelectedItem.ToString();

                Frame.Navigate(typeof(uredi_vrt));
            }
            catch
            {
                napaka.Text = "Izberi vrt";
            }
        }
    }
}
